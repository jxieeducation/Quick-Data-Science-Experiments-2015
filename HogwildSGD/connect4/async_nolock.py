...
def update(w, x, y, alpha):
    new_w = np.frombuffer(w)
    p = predict(new_w, x)
    new_w -= alpha * (p - y) * np.append(x, 1)

def batch(w, X, y, alpha):
...
def train(X, y, alpha):
    ...
    n, m = X.shape
    w = mp.RawArray(ctypes.c_double, m + 1)
    x_list = []
    y_list = []
    ...
    procs = [Process(target=batch, args=(w, x_list[i], y_list[i], alpha)) for i in range(thread_num)]
    start = time.time()
    for p in procs: 
        p.start()
    for p in procs: 
